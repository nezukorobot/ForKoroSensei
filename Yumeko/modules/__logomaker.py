...
async def lego(event):
 ...
 try:
    ...
    fname = "luna.png"
    img.save(fname, "png")
    await telethn.send_file(event.chat_id, file=fname, caption = f"Made by [Luna ✨](https://t.me/KoroXSensei_bot)")         
    await pesan.delete()
    if os.path.exists(fname):
            os.remove(fname)
 except Exception as e:
    await event.reply(f'Error, Report at koro sensei support chat')
